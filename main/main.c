...
void task1(float *accel, float *gyro) {
    ...
    while (1) {
        ...
        esp_err_t ret = i2c_master_transmit(dev_handle, &start_reg, 1, pdMS_TO_TICKS(100));
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "I2C transmit failed: %X", ret);
            // esp_task_wdt_reset();
        }

        ret = i2c_master_receive(dev_handle, data_rd, 12, pdMS_TO_TICKS(100));
        if (ret != ESP_OK) {
